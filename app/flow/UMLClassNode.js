...
const saveChanges = (type, index = null) => {
  ...
  if (type !== 'attributes' && type !== 'methods') {
    ...
    setEditState(prevState => ({
      ...prevState,
      [type]: prevState[type].map((item, idx) => idx === index ? true : false),
    }));
  }
};
  ...
  return (
    ...
    {/* Existing code for handles and the name field */}
      ...
      {editState.name ? (
        ...
        <div className="title nodrag" onDoubleClick={() => toggleEditState('name')} {...longPressProps}>
          {editableData.name}
        </div>
      )}
    <hr />
    <div>
      ...
      <ul>
        ...
        <button onClick={addAttribute} className="addButton">Add Attribute</button>
      </ul>
    </div>
    <div>
      ...
      <ul>
        ...
        <button onClick={addMethod} className="addButton">Add Method</button>
      </ul>
    </div>
